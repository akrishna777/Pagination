...
function paginate(arr, eachPageSize) {
    return arr.reduce((acc, currentData, totalData) => {
        let noOfPages = Math.floor(totalData / eachPageSize)

        var page = acc[noOfPages] || (acc[noOfPages] = [])
        page.push(currentData)
...
let page_size = 10;
allPages = paginate(userData, page_size)


/* Generate pages for Pagination Bar */
let pageLi = ""
for (let i = 0; i < allPages.length; i++) {
    pageLi += '<li onclick="pageChange(' + i + ')" class="page-item" id="page_' + i + '"><a class="page-link" href="javascript:void(0)">' + i + '</a></li>'
}


/* Add pages to Pagination Bar */
let PaginationBar = document.getElementById('pagination')
PaginationBar.innerHTML = `
<li class="page-item"><a class="page-link" onclick="First_Page()" href="#">First<a><li>
...
<li class="page-item"><a class="page-link" onclick="Last_Page()" href="#">Last<a><li>
`


/* Function to populate new data on selected page */
function pageChange(pageNumber) {

    /* Add active class to selected page */
    let selectedPage = document.getElementById(`page_${pageNumber}`)
    console.log(selectedPage)
    selectedPage.classList.add("active")


    /* Remove active class from unselected page */
    let activePages = document.getElementsByClassName('active')
    for (let i = 0; i < activePages.length; i++) {
        if (activePages[i] !== selectedPage) {
            activePages[i].classList.remove('active')
        }
    }


    /* Clear table body and populate new data on selected page */
    let tableBody = document.getElementById("table").getElementsByTagName('tbody')[0]
    tableBody.innerHTML = " "
    currentPage = selectedPage.firstElementChild.innerHTML
    let newPage = allPages[currentPage]

    printRows(newPage)



    /* Print the Rows in table body */
...
function previousPage() {
    if (currentPage < allPages.length && currentPage != 0)
...
function Last_Page() {
    ...
    currentPage = parseInt(lastIndex)
    pageChange(currentPage)
}
